...
# Save mappings
pd.DataFrame(repo_mapping.items(), columns=["original_repo", "new_repo"]).sort_values("new_repo").to_csv(OUTPUT_REPO_MAPPING_FILE, index=False)
pd.DataFrame(user_mapping.items(), columns=["original_user", "new_user"]).sort_values("new_user").to_csv(OUTPUT_USER_MAPPING_FILE, index=False)
...
df_new_edges = df_new_edges.sort_values(["repo", "actor"])  
...
df_features = df_features[feature_columns].sort_values("new_repo_index")  
